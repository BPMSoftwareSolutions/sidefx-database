...
export async function migrateCapabilityPreparation({ dryRun = false } = {}) {
  const id = '005-capability-preparation';
  ...
  try {
    ...
    if (!old) {
      for (let i = 0; i < batches.length; i++) {
        ...
        catch (error) { throw new Error(`CAPABILITY_PREPARATION_BATCH_${i}: ${error.message}`); }
      }
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  try { console.log(JSON.stringify(await migrateCapabilityPreparation({ dryRun: process.argv.includes('--dry-run') }))); }
