...
async function select(tx,definitionPk,bundle,label){
 ...
 const bindingId=await bindAsset(tx,{requirementId:row.id,revision:bundle.revision,altText:label,presentation:{scope:'DECLARED_SOURCE_BOUNDARY',playback:'ONLY_AUTHORED_TRACES'}});
 if(!row.selected||row.selectedKind==='CIRCUIT_BUNDLE')await approveAndSelect(tx,{bindingId,reviewer:'SideFX SCL compiler and source validation',reason:'Exact capsule lineage and scenario ownership match. Canonical grammar, geometry and junction contact checks passed. Declared structure only; no authored flow is invented.'});
}
try{
 ...
 for(const cap of index.capabilities){
  ...
  const entries=await transaction(pool,async tx=>{
   ...
   const capBundle=await importAsset(tx,{key:'scl/capability/'+cap.definitionPk,kind:'CIRCUIT_BUNDLE',bytes:jsonBytes({version:1,definitionPk:cap.definitionPk,source:index.source,capsuleDigest:cap.capsuleDigest,members:all.map(f=>({path:f,revision:cache.get(f).revision}))}),mediaType:'application/json',definitionPk:cap.definitionPk,provenance:{compiler:'content-lab-scl',scope:'DECLARED_SOURCE_BOUNDARY'}});
   ...
   for(const scenario of cap.scenarios){
    ...
    const bundle=await importAsset(tx,{key:'scl/scenario/'+scenario.definitionPk,kind:'CIRCUIT_BUNDLE',bytes:jsonBytes({version:1,definitionPk:scenario.definitionPk,source:index.source,entry:scenario.entry,members:files.map(f=>({path:f,revision:cache.get(f).revision}))}),mediaType:'application/json',definitionPk:scenario.definitionPk,parents:[{revision:capBundle.revision,role:'CAPABILITY_SOURCE'}],provenance:{compiler:'content-lab-scl',scope:'DECLARED_SOURCE_BOUNDARY'}});
    ...
    rows.push({capabilityId:cap.id,capabilityDefinitionPk:cap.definitionPk,scenarioId:scenario.scenarioId,definitionPk:scenario.definitionPk,objectPk:scenario.objectPk,bundleRevision:bundle.revision,label:scenario.label,entry:scenario.entry,publicFiles:[scenario.entry,scenario.data,...cap.original.filter(f=>/\.(js|css|html)$/.test(f)),...scenario.files.filter(f=>f.includes('/textures/'))]});
   }
   return rows;
  });
